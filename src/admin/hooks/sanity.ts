...
export const useTriggerSanityProductSync = (
  ...
  return useMutation({
    ...
    onSuccess: (data: any, variables: any, context: any) => {
      queryClient.invalidateQueries({
        queryKey: [sanitySyncQueryKey.details(), sanityDocumentQueryKey.detail(id)]
      });
...
export const useSanityDocument = (
  ...
  const fetchSanityProductStatus = async (
    query?: Record<any, any>,
  ) => {
    return await sdk.client.fetch<Record<any, any>>(`/admin/sanity/documents/${id}`, {
      query,
    });
  };
